...
SERIES = {
    "i-kissed-the-stones": {
        ...
        "ambiguous_ok": [
            ...
            ("ch11", "你对我好，一直都好"), ("ch11", "可是你看我的时候是空的"),
            ("ch11", "你不用对不住"), ("ch11", "你又没有骗我"),
            ("ch11", "那对你不好"), ("ch11", "镇上会说你"),
            ("ch11", "可要是那个人真的值得你走这一步"), ("ch11", "——你叫他小心点"),
            # Ch18 父亲退场那一句：
            ("ch18", "你母亲每个礼拜四"),
            # Ch19 Lorenzo 在巷子口：
            ("ch19", "你那位"),
            # Ch20 书房里父亲说的那一段（那一场通篇只写「他」）：
            ("ch20", "你别等我说那句话"), ("ch20", "我十六岁那年给你定下的事"),
            ("ch20", "十八岁你翻窗子走了"), ("ch20", "我算不过你"),
            ("ch20", "这一笔我算不过你"),
            # ── Ch21《你》= 第②把锁开的那一章，以下三句「你」全是刻意的 ──
            # ① 他对徒弟 Beppe 说的（不是对她，他对徒弟本来就用「你」）：
            ("ch21", "你以后会起好几千块"),
            # ② ⭐ 全系列第一次对她说出「你」——圣约翰之夜，他自己都没察觉说漏了：
            ("ch21", "你拿了？"),
            # ③ 那一夜之后他改不回去的那两句（锁已经开了）：
            ("ch21", "……你头发上全是水"),
            ("ch21", "我爱你，Winter"),
            # ④ 七月那个下午，Ch17「您弹您的，气我来」只换掉一个字：
            ("ch21", "你弹你的"),
        ],
        # 叙述（引号外）里不许出现的第一人称
        # ⚠️ 第二人称叙述里绝不该出现的第一人称（引号内、`>` 引语、c-note 卡片都不算）
        "narration_forbid": ["我父亲", "我母亲", "我们站", "我们两个", "我们进去", "我们走到", "我们回", "我旁边", "我家里那",
                             "看了我一眼", "看我一眼", "对我说", "我说人家",
                             "我问他", "我走过去", "我站在"],
                ...
                if not allow:
                    for ctxw in rule.get("allow_if_near", []):
                        if ctxw in near:
                            ...
                            break
                if not allow:
                    m = re.search(r"(?:ch|chapter-)(\d+)", path.split("/")[-1])
                    num = m.group(1) if m else ""
                    for ch, pref in cfg.get("ambiguous_ok", []):
                        if ch.replace("ch", "") == num and q.startswith(pref[:12]):
                            allow = True
                            break
                if not allow:
