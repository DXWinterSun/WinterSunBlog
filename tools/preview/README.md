...
| `--eyebrow` | 顶部状态条里「状态」那一格，默认「草稿 / 待冬璇过目」 |
| `--kicker` | 大标题上方那行斜体系列名，如 `Everybody's Home · Sam Bell AU` |

**页头长这样**（2026-09 按 Winter 的观感改的，她明确说这一版比朴素版好看）：
顶部一条带发光圆点的**状态条**（状态 / 系列 / 章节，分隔线由 CSS 生成，换行不会在行尾吊竖线）
→ 斜体 `--kicker` → **Volkhov 衬线大标题**（`--title` 建议填英文篇名）→ 中文副标 → mood 一行。
小节标题是**斜体衬线 + 右侧一条渐隐横线**。字体走 Google Fonts（Volkhov / Noto Serif SC /
EB Garamond），Artifact 的内容安全策略允许这两个域名，别换成别的字体源。
...
- `### 小节标题`
- `> 引用块`（文首题词、家书 / 公告、文末收束引语）。**块里只写一个 `>` 的空行 = 块内换段**，
  不是引用结束——家书那种「一条一段」的东西全靠它。（2026-09 修：早期版本会把一整封家书
  切成好几个方框，中间还漏出光秃秃的 `>`。）
- `**粗体**`
- `---` 分隔线
...
- 页面除 Google Fonts 那一个 `<link>` 外自包含（CSS 内联）。字体域名是 Artifact 白名单里的，
  换成别的 CDN 会被静默拦掉、页面变回系统字体。
