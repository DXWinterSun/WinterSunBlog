...
| 情况 | 那句话 |
|---|---|
| 只看某人 ＋ 他别的类里有东西 | 现在 **Justin Hammer** 还没有碟片，不过他有 [🃏 卡牌 2] 和 [📔 场刊]，[去看看？] |
| 只看某人 ＋ 他真的一件都没有 | 现在还没有 **X** 的东西——[先去画册看看这张脸？]（馆长多一个 [给他添一件]） |
| 搜索 / 类别把东西筛没了 | 架子上还没有叫「xxx」的东西——[看看全部 60 件？] |
...
- 「还没有 ___」里那个空格由 `lackName()` 填（类别名 / 带签名的东西 / 进了 NAS 的东西 /
  叫「xxx」的东西），⚠️ 别直接拿 `filterName()` 去拼，会写出「还没有有签名的」。
