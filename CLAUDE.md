...
横幅）；视图 / 排列偏好存 `ws-sam-shelf-view`。**每保存一次就会触发一次约 40 分钟的
站点重建**，所以连着加好几件时不必每次都等——但别在重建期间反复刷新页面找东西。
...
（2026-09-11 实测：10:51 开始 11:34 结束，**42 分钟**；早先记的「约 22 分钟」已偏低，别照那个等）。而 Pages 的机制是**「新推送会取消正在跑的旧构建」**
