...
⚠️⚠️ **点卡片 = 翻面，绝不是「进编辑」**（Winter 2026-09-22：「我想欣赏动态的时候
总是会误触到编辑……我一般都是在手机端用的」）。馆长的编辑收在卡片右上角那颗 `✎`
（`.item__edit` / 清单里的 `.row__edit`，`body.is-curator` 才显示），点它要
`stopPropagation`。**以后不要再把整张卡 / 整条清单绑成编辑热区**——手机上那是个
到处都会误触的雷。卡面上的划动（卡牌倾斜）也不该触发翻面：按下与抬起之间位移
超过 8px 就当成在划，不翻。
