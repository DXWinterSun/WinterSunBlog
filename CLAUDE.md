...
### ⚠️ `No One Walks Off`（Bill Greaves AU）的炽恋章 = 默认「完全吃爽」这一更狠的档

Winter 明确要求：**这个系列以后所有 `炽恋` 章，一律默认写到「完全吃爽、火力全开」的更露骨档，
不用再问、也不要自己缩水。基准参照第 50 章 `Coach`**
（`_posts/2026-09-02-no-one-walks-off-chapter-50-coach.md`）。比通用「那办吧」档更进一步：
...
- **但仍要保住这个系列的两条命根**：① **声口**——Bill 一本正经的教练腔 / 贫嘴 / 嘴硬心软不能丢，
  要「又狠又好看」，不是器官报告；② **称呼动力学**——对白称呼仍是英文（`daddy` / 情趣暗号 `Coach`），
  narration 里 `你爸/老爸` 保持中文；情感戏要有互相称呼。
- **红线照旧**：她 18 岁后的成年恋才写到这一档；此前只克制暗涌。其余全站规则（弯引号、summary ≤35
  且无 ASCII 双引号、身份标签齐、de-sin 去中式）一样不能松。

一句话：**No One Walks Off 的辣章，默认就是第 50 章那一档往上，别缩。**
