...
| 深夜热线 `sam/hotline/`（2026-08 加：选一个角色进聊天室发预设消息，他按消息情绪从自己 5 句台词里挑一句回复；对话历史存本机 localStorage 键 `ws-hotline-v1`） | `sam/lines.json`（运行时 fetch，按台词 `kind` 分桶：锚 / 情话系 / 暗告白系） | 无——lines.json 加好新角色，通讯录自动多一个号码 |
