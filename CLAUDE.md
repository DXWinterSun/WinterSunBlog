...
- 她的方案（2026-09-26 定）：起点 70.05 kg、173 cm、26 岁（11 月 6 日满 27），
  每天消耗约 2000 千卡，**上限 1350 千卡**（她选的「冲一冲」档，想生日前摸到 65）。
  运动了她会说，那天按运动量多给一点（快走半小时约 +150）。
  她报的运动记进 `workouts/<日期>`（`date`/`items[]`/`kcal`），网页暂不显示，只给你回头看。
  仰卧起坐这类小运动消耗太小，**不加饭量**（她同意的）。
  她说「小奶茉」＝霸王茶姬小奶茉无糖，**一杯 170 千卡**，直接按这个记，别让她有负担。
