...
🕳️ **筛空了要给出路，别只说「换个条件」**（Winter 2026-09-23：「我也是看见他有藏品
才只看他的，结果上来一看这一类是空的，是不是可以有个引导」）。`fillEmpty()` 先把
**当前同时开着的条件**逐条写出来（只看某人 ＋ 某类 ＋ 搜什么），再摆几颗**先算好件数**
的出路钮：「看他的全部 N 件」（只松开类别与搜索）·「看全部某类（N 件）」（只松开角色）·
「看全部 N 件」。角色确实一件都没有时，给的是「去画册看看这张脸」＋（馆长）「给他添一件」。
⚠️ 每颗钮的件数都是真算过的，不许点过去又是一片空。
