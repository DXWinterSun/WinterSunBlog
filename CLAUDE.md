...
- **标签的疏密分三档**（`_header.scss`，Winter：「现在这个标签少了，但是电脑上都挤在一起🤔」）：
  ≥1040px 最宽松（间距 14px、左右内边距 22px）／901–1039px 中档（6px / 14px）／
  ≤560px 最紧（1px / 7px，一行装得下）。⚠️ **901–1039 那一档不能用大间距**——
  站头同一行里还挤着头像、180px 的搜索框和两颗小圆钮，950px 宽时整页会被撑出
  63px 的横向滚动条（实测过）。改这里之后照着 390 / 768 / 910 / 950 / 1000 /
  1040 / 1200 / 1440 逐个量一遍「整页横向溢出」，每个都要是 0。
